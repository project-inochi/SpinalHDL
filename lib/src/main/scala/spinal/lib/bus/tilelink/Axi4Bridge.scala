...
class Axi4Bridge(p : NodeParameters, withAxi3 : Boolean = false, forceAxi4Len : Boolean = false) extends Component{
  ...
  val d = new Area{
    ...
    val responseOpcode = arbiter.io.chosen(0).mux(Opcode.D.ACCESS_ACK_DATA(), Opcode.D.ACCESS_ACK())
    ...
    io.up.d.corrupt := arbiter.io.output.denied && Opcode.D.isData(responseOpcode)
