...
//  io.down.cmd.mask      := io.up.a.mask
  ...
  val responseOpcode = io.down.rsp.context(0).mux(tilelink.Opcode.D.ACCESS_ACK, tilelink.Opcode.D.ACCESS_ACK_DATA)
  io.up.d.opcode  := responseOpcode
  io.up.d.param   := 0
  ...
  io.up.d.corrupt := io.down.rsp.isError && tilelink.Opcode.D.isData(responseOpcode)
}
